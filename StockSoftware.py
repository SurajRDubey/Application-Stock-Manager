...
labelFrame = ttk.LabelFrame(Inventory,text="MANAGE STOCK")
...
def Bill_add():
	db = sqlite3.connect('Stock_Info.db')
...
Billing_PRODUCT_QUANTITY.grid(column=0,row=3,sticky='W')
Billing_PRODUCT_QUANTITY_Entry = ttk.Entry(BillingFrame,textvariable=BILL_QT)

Billing_PRODUCT_QUANTITY_Entry.grid(column=0,row=2,sticky='w')
...
tree1.grid(row=11, columnspan=4, sticky='nswe')
